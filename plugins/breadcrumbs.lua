...
M.setup = function()
  ...
  if lvim.builtin.breadcrumbs.on_config_done then lvim.builtin.breadcrumbs.on_config_done() end
end
...
M.get_filename = function()
  ...
  if not f.isempty(filename) then
    ...
    if f.isempty(file_icon) then file_icon = lvim.icons.kind.File end

    local buf_ft = vim.bo.filetype

    if buf_ft == "dapui_breakpoints" then file_icon = lvim.icons.ui.Bug end

    if buf_ft == "dapui_stacks" then file_icon = lvim.icons.ui.Stacks end

    if buf_ft == "dapui_scopes" then file_icon = lvim.icons.ui.Scopes end

    if buf_ft == "dapui_watches" then file_icon = lvim.icons.ui.Watches end
...
local excludes =
  function() return vim.tbl_contains(lvim.builtin.breadcrumbs.winbar_filetype_exclude or {}, vim.bo.filetype) end
